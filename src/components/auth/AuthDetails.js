        ...
        <div className='font-semibold text-sm md:text-base flex h-full group w-full content-center place-self-end text-black transition-all ease-in-out duration-700'>{authUser ?
            <>
                <div className='items-end rounded h-full bg-zinc-500 z-50 border-theme border-black p-1 px-2 group-hover:bg-zinc-400 drop-shadow-lift-hard transition-all ease-in-out duration-700'>
                    <p className='toolitem'>{authUser.email}</p>
                    <div className='toolitem absolute top-8 right-0 group-hover:bg-zinc-500 border-theme border-black rounded opacity-0 group-hover:opacity-100 justify-around flex flex-col w-full bg-zinc-500 overflow-hidden h-0 group-hover:h-60 transition-all ease-in-out duration-700'>
                        ...
                        <button className='toolitem p-2 m-2 rounded text-left bg-zinc-500 hover:bg-zinc-300 border-theme border-zinc-500 hover:border-black hover:drop-shadow-lift-hard transition-all ease-in-out duration-100' onClick={userSignOut}>Log out</button>
                    </div>
                </div>
            </>

            :
            <div className='rounded w-24 bg-zinc-500 z-50 border-theme border-black p-1 px-2 group-hover:bg-zinc-400 drop-shadow-lift-hard transition-all ease-in-out duration-700' >
                <p className='toolitem text-center'>Signed Out</p>
            </div>}
        </div >
    )
}
